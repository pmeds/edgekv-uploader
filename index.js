...
var edgeGrid = new EdgeGrid({
  path: '~/.edgerc',
  section: 'default'
});

console.log(edgeGrid)
//console.log( "Hello!" );
...
async function initEdgeKv(edgeGrid, parameters) {
  ...
  if (response.statusCode == 200){
    ...
    console.log(`Unexpected status code: ${response.statusCode}: ${response.body}`, response);
  }
} catch(error){
    ...
    fs.createReadStream(csvFile)
      ...
      uploader = async () => {
        ...
        if (response.status == 200){
          ...
          console.error(`Failed to upsert data to ${network} with key: ${key}.  Unexpected status code: ${response.statusCode}: ${response.body}`, response);
        }
      };
...
async function main() {
  ...
  await initEdgeKv(edgeGrid, apiParameters);
  //await createKvNamespace(eg, argv.namespace, apiParameters)

  await upsertData(argv.csv, argv.key, edgeGrid, argv.namespace, argv.group, apiParameters);

  if (argv.generateToken) {
    kvAccessToken = await generateKvAccessToken(edgeGrid, argv.namespace, apiParameters);
  }
